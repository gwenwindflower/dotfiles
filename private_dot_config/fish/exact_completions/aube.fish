# @generated by usage-argv for `aube __complete_word__ --shell fish`

function __usage_complete_aube
    ...
    set -l out (command 'aube' __complete_word__ --shell fish --line "$line" 2>/dev/null)
...
complete -c 'aube' -f -a '(__usage_complete_aube)'
# @generated by usage-argv for `aubr __complete_word__ --shell fish`

function __usage_complete_aubr
    set -l line (commandline -cp)
    # `commandline -cp` is already cut at the cursor, so there is nothing to say about where
    # the cursor is: the end of what it gives is where the cursor was.
    set -l out (command 'aubr' __complete_word__ --shell fish --line "$line" 2>/dev/null)
    # Built with printf rather than written literally: fish's `case` takes patterns, not
    # computed values, and a control byte is not something to spell twice.
    set -l marker_any (printf '\x01files')
    set -l marker_dirs (printf '\x01dirs')
...
# `-f` so fish offers no filenames of its own: this CLI says when they belong, and the function
# produces them itself when they do.
complete -c 'aubr' -f -a '(__usage_complete_aubr)'
# @generated by usage-argv for `aubx __complete_word__ --shell fish`

function __usage_complete_aubx
    set -l line (commandline -cp)
    # `commandline -cp` is already cut at the cursor, so there is nothing to say about where
    # the cursor is: the end of what it gives is where the cursor was.
    set -l out (command 'aubx' __complete_word__ --shell fish --line "$line" 2>/dev/null)
    # Built with printf rather than written literally: fish's `case` takes patterns, not
    # computed values, and a control byte is not something to spell twice.
    set -l marker_any (printf '\x01files')
    set -l marker_dirs (printf '\x01dirs')
...
# `-f` so fish offers no filenames of its own: this CLI says when they belong, and the function
# produces them itself when they do.
complete -c 'aubx' -f -a '(__usage_complete_aubx)'
