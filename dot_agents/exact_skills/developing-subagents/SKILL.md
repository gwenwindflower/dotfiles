...
description: Design subagents — specialized assistants with their own system prompt, model, and permissions. Use when adding files under .claude/agents/, .opencode/agents/, or equivalent platform paths, editing subagent frontmatter, or scaffolding new subagents.
---

# Developing Subagents

Subagents isolate verbose work from parent context, encode repeated workflows, and route narrower tasks to cheaper models. They can't spawn other subagents — chain from the main conversation, or compose via Skills.
...
If a Skill carries the same instructions, prefer the Skill — Skills compose; subagents don't.

## Description: routing metadata

The description is the only field the routing model sees when deciding to invoke. Treat it like a Skill description.
...
**Cut:** numbered `(1)…(2)…` lists, "always load", time-stamped guidance, restating the agent name, comparisons to platforms or tools the user isn't using.
...
The body is the agent's full system prompt — every line loads on every invocation.
...
Frontmatter earns its line when it does something the platform's global settings don't already do. Read the global config before adding any permission entry — restating rules already in `~/.claude/settings.json` or `~/.config/opencode/opencode.jsonc` is dead config that drifts from source.
