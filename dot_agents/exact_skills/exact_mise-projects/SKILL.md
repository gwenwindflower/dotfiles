...
- **Tools float on `latest`.** `mise.toml` declares the toolchain for CI and contributors; release cooldowns belong to each machine's global mise config, never to the project. Pin a version only when the project needs that version, and say why in `mise.toml`. The core set (git-cliff, pinact, zizmor, shellcheck, prek, rumdl, rust, cargo-binstall) is installed globally on the maintainer's machine, so the bootstrap writes `mise.local.toml` disabling all of it; only a genuinely project-specific tool gets a project-scoped install.
