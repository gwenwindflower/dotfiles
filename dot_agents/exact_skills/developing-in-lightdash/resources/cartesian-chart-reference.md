...
3. **Stacking**: Use the same `stack` value for series you want stacked. Only bar and area charts support stacking.

4. **Dual Y-axis**: Use `yAxisIndex: 0` for left axis, `yAxisIndex: 1` for right axis.

5. **Horizontal bars**: Set `flipAxes: true` in layout.

6. **Pivot data**: Use `pivotValues` in `yRef` to create series from pivoted dimensions.

7. **Reference lines**: Add `markLine` to series for targets or thresholds.

8. **Grid spacing**: Use `grid.containLabel: true` to prevent label clipping.
