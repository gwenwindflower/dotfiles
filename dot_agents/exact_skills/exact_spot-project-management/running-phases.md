...
Executing one Phase in one session, on one branch, in one worktree. The flow is platform-neutral — Claude Code, Codex, and OpenCode differ only in how helpers are spawned. In Linear mode the Phase is a parent issue; the substitutions are in [linear-planning](linear-planning.md#run-a-parent-issue), everything else here applies as written.
...
1. Read `SPEC.md` (goals, non-goals, vocabulary), the Phase block in `TODO.md` (or the parent issue and its sub-issues), and each listed requirement ID's wording in its durable spec (`SPEC.md` for `R<NNN>`, `specs/<dom>-*.md` for `<dom>-R<NNN>`).
2. Confirm the Phase is unblocked — every Phase on its `**Dependencies**:` line is in DONE, or every blocking issue is Done.
...
Clean → stage that work plus its TODO checkoff (or add the sub-issue's `Closes` line) and commit. Off → send it back with sharper direction. Don't patch material drift on top; a redo from a sharpened brief produces a cleaner result than surgery on a bad diff.
...
- **Squishy requirements** ("feels responsive", "helpful errors") — implement, review, iterate. Document the judgment call in the ledger (`DONE.md` or the parent issue's closing comment).
...
2. Move the Phase block from `TODO.md` to `DONE.md`: header verbatim with ✅, Objectives and Tasks verbatim with checked boxes, and a short narrative — decisions, surprises, anything a future reader traces back here. Linear: the same narrative goes in a closing comment on the parent issue.
3. Update `docs/` for current-state changes; append surfaced harness/tooling needs to the right `dev-*` spec.
4. **Amend the close into the Phase's last commit** — stage the TODO/DONE/doc edits and `git commit --amend --no-edit`. The close is a checkbox, not an announcement: no close commit, no Phase trailer. `DONE.md` is the ledger. Linear: doc edits still ride the last commit, which also carries `Closes <parent ID>`.
