...
```bash
pnpm install @modelcontextprotocol/ext-apps @modelcontextprotocol/sdk zod
pnpm install -D tsx vite vite-plugin-singlefile
```

Use `pnpm install` to add dependencies rather than manually writing version numbers. This lets pnpm resolve the latest compatible versions. Never specify version numbers from memory.
