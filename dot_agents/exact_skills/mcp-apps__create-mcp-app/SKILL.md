...
```bash
git clone --branch "v$(pnpm view @modelcontextprotocol/ext-apps version)" --depth 1 https://github.com/modelcontextprotocol/ext-apps.git /tmp/mcp-ext-apps
```
...
- `package.json` with `pnpm run` scripts and required dependencies
...
**Always** use `pnpm install` to add dependencies rather than manually writing version numbers:

```bash
pnpm install @modelcontextprotocol/ext-apps @modelcontextprotocol/sdk zod express cors
pnpm install -D typescript vite vite-plugin-singlefile concurrently cross-env @types/node @types/express @types/cors
```

This lets pnpm resolve the latest compatible versions. **Never** specify version numbers from memory.
...
pnpm pkg set scripts.dev="cross-env NODE_ENV=development concurrently 'cross-env INPUT=mcp-app.html vite build --watch' 'tsx --watch main.ts'"
```
