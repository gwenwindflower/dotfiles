...
Do not add commits purely for updating the TODO.md. If forced to, or dealing with a situation where subagents have added commits like `docs(todo): mark phase 1 as complete`, make sure to rebase and squash those commits back into the changes that completed the related work. This keeps the commit history clean and meaningful: the commits show the task completion, rather than also having noise about the list itself.
