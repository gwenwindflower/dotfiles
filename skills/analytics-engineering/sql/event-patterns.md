...
Event data has an inherent order (timestamp, user, session). Aggregations alone lose that order. Reach for these patterns when the question involves *sequence*: "what did users do before X", "how long between A and B", "which sessions contain pattern P", "where do users drop off", "find runs of consecutive Y".
...
All examples here follow [effective-sql](effective-sql.md): lowercase keywords, CTE pipelines (import → transform → output), explicit `as` aliases, type/unit suffixes on column names, `select * from <final_cte>` to close.
