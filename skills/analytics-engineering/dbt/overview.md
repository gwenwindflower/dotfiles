...
Apply software engineering discipline to analytics code. Read [effective-sql](../sql/effective-sql.md) first, and follow its style unless the project has established patterns that override it.
...
- Keep SQL as CTE pipelines ending in `select * from final`; follow [effective-sql](../sql/effective-sql.md) for detailed style.
