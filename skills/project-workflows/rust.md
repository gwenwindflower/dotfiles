...
README install order for a standalone CLI: Homebrew (when the tap is on), `cargo binstall`, `cargo install --git ... --locked`, release archive. [releasing](releasing.md) has the matrix for every language.

## Herdr plugins

Plugins add `herdr-plugin.toml` to `version:files` and a manifest rewrite to `version:write`, keep the exact-version installer from [herdr-plugins/project-shape](herdr-plugins/project-shape.md), and disable the `compile` binstall strategy so the installer's fallback owns compilation.
