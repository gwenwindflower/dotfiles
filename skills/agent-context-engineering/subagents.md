...
- [Claude Code](subagents-claude-code.md) — `.claude/agents/`, frontmatter fields, permission modes, hooks, skills, memory, MCP scoping
- [OpenCode](subagents-opencode.md) — `.opencode/agents/`, primary/subagent modes, per-tool allow/ask/deny, cascade rules, JSON form
- [Codex](subagents-codex.md) — `.codex/agents/`, TOML custom agents, explicit parallel spawning, inherited sandbox/config, global `[agents]` limits
