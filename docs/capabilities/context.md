...
Manage external skills through `gh skill`: install/add, update, search, preview, and list are routine within task scope, including user scope. They run `open` in Claude, because `~/.agents/skills` is a protected path in its sandbox, and `sandboxed` in Codex, which grants that directory. Removal is `review-open`. Forced replacement is `user-open` in Claude and `review-open` in Codex. `gh skill publish` is `deny`; releases run through the project release task. Install with `--agent universal --scope user` so skills land in `~/.agents/skills`; own skills live in the dotfiles `skills/` tree and edits return there through `skillet save <name>`. Do not use package-runner `skills` commands or alternative skill installers.
