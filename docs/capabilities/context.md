...
Manage external skills through `gh skill`: install/add, update, search, preview, and list are routine within task scope, including user scope. Review destructive removal or forced replacement; raw skill publishing follows the project release-task boundary. Install with `--agent universal --scope user` so skills land in `~/.agents/skills`; own skills live in the dotfiles `skills/` tree and edits return there through `skillet save <name>`. Do not use package-runner `skills` commands or alternative skill installers.
