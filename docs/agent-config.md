...
| Process containment | Native Bash filesystem/network sandbox; explicit host exclusions for selected commands | `workspace-write`, with scoped additional writable roots and networking enabled | No process filesystem or network sandbox is supplied by this config; shell runs with host access |
| Sensitive files | `Read`/`Edit` denies protect file tools and merge into the Bash sandbox; excluded host commands lose that process boundary | General filesystem reads; sensitive access is governed by guidance and review, without profile read-deny enforcement | `read`/`edit` denies protect file tools; they do not establish shell containment |
...
Codex runs one of two sandbox systems, and `symsources/codex/config.toml` is whichever one is active. `workspace-config.toml` holds the legacy `sandbox_mode = "workspace-write"` layout, which is known to work and is the fallback when a priority project hits a rough edge; `profile-config.toml` holds the permission-profile layout (`default_permissions = "dev"`, `[permissions.dev]`), which is the target and gets iterated on when there is slack. The two sources differ only in the sandbox block; everything else is kept identical across both. Swapping is a copy of the chosen source over `config.toml` followed by a fresh Codex session. Before a swap, diff the live file against its source and port Codex-written state (project trust, plugin cache versions) into both sources so nothing is lost. Hand edits to shared settings land in both sources and the live file; sandbox edits land in the matching source and the live file when it is active. This split is temporary until the profile system is reliable enough to be the only source.

The capability docs below describe the workspace-write layout while it is active. Codex uses `sandbox_mode = "workspace-write"` and `[sandbox_workspace_write]`; `default_permissions`, `[permissions]`, and the profile network proxy are absent. Additional writable roots cover tool caches/state, the notes vault, and real symlink targets. Reads and outbound networking are generally available. Approval policy remains `on-request` with `auto_review`; command rules and task authority still apply. Start a fresh session after configuration changes. [Codex configuration reference](https://developers.openai.com/codex/config-reference).
...
| --- | --- |
| [Workspace](capabilities/workspace.md) and [network](capabilities/network.md) | Use Codex workspace-write with scoped state/cache writes, general reads, and enabled networking. Keep OpenCode's lack of an OS sandbox explicit. |
...
Codex writable roots include Go telemetry, local search caches, and the real chezmoi skill-lock manifest. Filesystem read-deny and domain-allowlist enforcement are not provided by this configuration. Verify workspace writes, cache writes, Git metadata access, and app IPC separately in the active session.

Codex loads the workspace-write configuration successfully, and its rule suite checks canonical operations and non-matches with all maintained files evaluated together. `tombi` reports two known upstream schema errors for integer agent limits. Config parsing and rule checks do not establish live sandbox or Obsidian IPC behavior.
