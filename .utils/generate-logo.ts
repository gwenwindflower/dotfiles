...
export async function generateLogo(
	...
	const mod = await import("text-to-svg");
	...
	for (const ch of text) {
		...
		if (d) {
			paths.push(
				`    <path class="${className}-glyph" data-char="${xmlEscape(
					ch,
	...
	const svg = `<svg viewBox="0 0 ${width} ${height}" class="${className}-svg" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${xmlEscape(
		text,
	)}">
