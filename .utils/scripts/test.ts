...
function bar(done: number, total: number): string {
  ...
  return `  ${green(FILLED.repeat(filled))}${dim(EMPTY.repeat(empty))}  ${
    dim(`${done}/${total}`)
  }`;
}
...
function parseOutput(
  raw: string,
): { passed: number; failed: number; failedTests: string[] } {
  ...
  for (const line of lines) {
    ...
    const sum = t.match(
      /(?:ok|FAILED)\s*\|\s*(\d+)\s*passed\s*\|\s*(\d+)\s*failed/,
    );
    if (sum) {
...
async function runSuite(
  ...
  { stdout: string; stderr: string; duration: number; success: boolean }
> {
...
async function main() {
  ...
  // Header
  write(
    `\n  ${bold(cyan(RUNNER_NAME))} test runner ${dim("·")} ${
      bold(String(suites.length))
    } suites\n\n`,
  );
...
async function runCompact(suites: Suite[], results: Result[]) {
  for (let i = 0; i < suites.length; i++) {
    ...
    const tick = setInterval(() => {
      ...
      write(
        `${up(2)}${CLEAR_LINE}  ${dim(SPINNER[frame])} ${suite.label}\n\x1b[1B`,
      );
    }, SPIN_MS);
    ...
    write(up(2));
    write(
      `${CLEAR_LINE}  ${icon} ${suite.label}  ${stats}  ${
        dim(fmtDuration(duration))
      }\n`,
    );
...
async function runPlain(suites: Suite[], results: Result[]) {
  for (let i = 0; i < suites.length; i++) {
    ...
    write(
      `  ${icon} ${suite.label}  ${parsed.passed} passed${failStr}  ${
