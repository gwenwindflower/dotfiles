...
async function createLabel(repo: string, spec: LabelSpec): Promise<void> {
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to create label '${spec.name}': ${res.status} ${await res
        .text()}`,
    );
  }
}
...
async function updateLabel(
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to update label '${currentName}': ${res.status} ${await res
        .text()}`,
    );
  }
}
  ...
  for (const [defaultName, targetName] of Object.entries(DEFAULT_LABEL_REMAP)) {
    ...
    try {
      ...
      record({
        area: "labels",
        level: "drift",
        message: `Could not remap '${defaultName}' → '${targetName}': ${
  ...
  for (const r of remote) {
    if (!manifestNames.has(r.name)) {
      record({
        ...
        level: "extra",
        message:
          `'${r.name}' is on the repo but not in the unified labels list`,
      });
    }
  }
}
  ...
  for (const spec of DISCUSSION_CATEGORIES) {
    ...
    if (drifts.length > 0) {
      record({
        area: "discussions",
        level: "drift",
        message: `Category '${spec.name}' needs manual edit: ${
          drifts.join("; ")
        }`,
      });
    }
